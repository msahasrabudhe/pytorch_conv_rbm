...
opt.data_dir_prefix = '../data/celeba_64x64_bicubic/'
opt.data_dir_prefix = './rs_rotated/'

# misc
opt.modelPath = ''#'results/checkpoints/CelebAnoHisteq_persProj_baseline_z64/wasp_model_epoch_92'
opt.dirCheckpoints   =   opt.output_dir_prefix + 'checkpoints/rsRotated_baseline_BCWarperCamera'
opt.dirImageoutput   =   opt.output_dir_prefix + 'images/rsRotated_baseline_BCWarperCamera'
opt.dirTestingoutput =   opt.output_dir_prefix + 'testing/rsRotated_baseline_BCWarperCamera'
opt.dirLogger        =   './tblogs/rsRotated_baseline_BCWarperCamera'
opt.vanillaLogger    =   './vlogs/rsRotated_baseline_BCWarperCamera'
...
encoders      = WaspNet.EncodersIntegralBCWarper(opt)
decoders      = WaspNet.DecodersIntegralBCWarperCamera(opt)
if opt.cuda:
...
for epoch in range(opt.epoch_iter):
    ...
    for iter_subset in range(5):
        ...
        for batch_idx, data_point in enumerate(dataloader, 0):
            ...
            dp0_z, dp0_zI, dp0_zP = encoders(dp0_img)
            dp0_I, dp0_output, dp0_ppars = decoders(dp0_zI, dp0_zP, baseg, ones_mat, ones_vec, zeroParams)
